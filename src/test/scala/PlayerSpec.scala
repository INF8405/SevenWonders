...
trait defaults {
  ...
  val defaultPlayer = Player(defaultHand.toSet, 3, Multiset(), Set(BATHS, PAWNSHOP))
}
...
class PlayerSpec extends Specification with defaults {
  ...
  "A Player" should {
    "receive 3 coins when they discard a card and it should be removed from their hand" in {
      defaultPlayer.discard(PRESS) === Player((defaultHand - PRESS).toSet, 6, Multiset(), Set(BATHS, PAWNSHOP))
    }
    ...
    "playing a card should:" +
      ...
      defaultPlayer.play(TAVERN, Map()) === Player((defaultHand - TAVERN).toSet, 7, Multiset(), Set(BATHS, ALTAR, TAVERN))
    }
    ...
    "cannot play a card he already has" in {
      val player = Player(defaultHand.toSet, 3, Multiset(), Set(BATHS, ALTAR, TAVERN))
    ...
    "total production" in {
      val player = Player(Set(THEATER, BATHS, ALTAR, PAWNSHOP), 3, Multiset(), Set(MINE, CLAY_PIT, GLASSWORKS))
